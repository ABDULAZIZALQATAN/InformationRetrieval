...
def getRetDf (corpus,exp , docs , terms):
    # WA-AX-0.4-30-15.csv
    # Extract Retrievability File based on input
...
def process(group,corpus,exp , docs , terms , b):
    ...
    fld = 'rel_count'
    dfTemp = df.groupby(group, as_index=False).agg({fld: 'sum'})
    Rel_count_g = relexp.compute_percent(dfTemp[fld])
    ...
    rel_count_exposure = relexp.compute_fairness_score(Rel_count_g,Exposure_g)
    ...
    result = [ str(x) for x in [rel_count_exposure,rel_exposure,size_exposure,Grp_exposure]]
    return result
